    ...
    public class Program
    {
        ...
        public string ChooseCity()
        {
            ...
            while (cityNum < 1 && cityNum > 6)
            {
                try
                {
                    cityNum = Console.Read();
                }
                catch
                {
                    Console.WriteLine("\nError: \" You write not exhist number, try again: \" ");
                    Console.Write("Your choosen nuber is: ");
                }
            }
        ...
        public async void WeatherShow(string cityName)
        {

            // Получаем погоду для выбранного города
            var weatherData = await GetWeatherNowAsync(cityName);            
            ...
            var forecastData = await GetWeatherAsync(cityName);
        ...
        public async  Task<WeatherData> GetWeatherNowAsync(string cityName)
        {
            string json = null;
            bool isTrue = true;
        ...
        public async  Task<ForecastData> GetWeatherAsync(string cityName)
        {
            string json = null;
            bool isTrue = true;
        ...
        public static void Main(string[] args)
        {
            ...
            prog.WeatherShow(prog.ChooseCity());
            Console.ReadKey();
        }
